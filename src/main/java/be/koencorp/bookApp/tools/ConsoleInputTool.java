...
/**
 * ConsoleInputTool A Utility class to get input from the user using the console
 *
...
public final class ConsoleInputTool {
  ...
  /**
   * Ask the user for a boolean(may repeat until input is correct). when the use leaves the input
   * blank the default value will be used if allowed.
   *
   ...
   */
  public boolean askUserYesNoQuestion(String question, boolean allowBlankDefault,
      boolean defaultValue) {
    do {
      System.out.print(question);
      String answer = keyboard.nextLine();
      answer = answer.toLowerCase();
      if (answer.equals("y") || answer.equals("yes") || answer.equals("j") || answer
          .equals("ja")) {
        return true;
      } else if (answer.equals("n") || answer.equals("no") || answer.equals("nee")) {
        return false;
      } else if (allowBlankDefault && answer.isBlank()) {
        return defaultValue;
      }
  ...
  /**
   * Ask the user for a boolean(repeat until input is correct). when the use leaves the input blank
  ...
  public <E extends Enum<E>> E askEnum(String question, Class<E> enumClass) {
    while (true) {
      try {
        String input = askUserString(question, 1);
        return Enum.valueOf(enumClass, input);
      } catch (Exception ex) {
        System.err.println("Error: input is not a enum of type " + enumClass.getSimpleName());
      }
    }
  }

  public String askUserRegex(String question, String regex) {
    ...
    while (true) {
      ...
      if (pattern.matcher(input).matches()) {
        ...
        System.err.println("Error: input does not match " + regex);
      }
    }
  }

  public UUID askUserUuid(String question) {
    while (true) {
      String input = askUserString(question, 36);
      try {
        return UUID.fromString(input);
      } catch (Exception e) {
        System.err.println("Error: input does not uuid");
      }
    }
  }
